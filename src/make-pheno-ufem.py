...
def make_argument_parser():
    ...
    parser.add_argument('--nhc', type=Path)
    parser.add_argument('--database', type=Path)
    parser.add_argument('--samples', type=Path, required=True)
    parser.add_argument('--output', choices=['patients', 'controls', 'unmerged'], default='patients')
    return parser
...
if args.output == 'controls':
    ...
    ufem_controls = ufem_samples[ufem_samples['nhc'].isna()]

    output = ufem_controls.copy()
    ...
    if args.nhc is None:
        parser.error('option --nhc is required')
...
if args.output == 'patients':
    ...
    output = ufem_samples[ufem_samples.nhc.notna()]
    output = pd.merge(output, ufem_nhc, left_on="nhc", right_on="FC SAP")
